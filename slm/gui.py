...
from slm.config import (
    Config, ModelType, RNNType, SamplingMethod, create_default_config
)
...
class ConfigurationFrame(ttk.LabelFrame):
    ...
    def create_widgets(self):
        ...
        ttk.Label(model_frame, text="Model Type:").grid(row=0, column=0, sticky='w', padx=5)
        self.model_type = ttk.Combobox(
            model_frame, 
            values=['rnn', 'transformer'], 
            state='readonly'
        )
        ...
        # RNN specific
        ttk.Label(model_frame, text="RNN Type:").grid(row=0, column=2, sticky='w', padx=5)
        ...
        param_configs = [
            ...
            ('embedding_dim', 'Embedding Dim:', self.config.model.embedding_dim, 32, 512),
        ...
        for i, (name, label, default, min_val, max_val) in enumerate(param_configs):
            ...
            if name == 'dropout':
                ...
                widget = tk.Spinbox(
                    params_frame, from_=min_val, to=max_val, 
                    increment=0.1, width=10, textvariable=var
                )
            else:
        ...
        train_configs = [
            ...
            ('learning_rate', 'Learning Rate:', self.config.training.learning_rate, 0.0001, 1.0),
        ]
        ...
        for i, (name, label, default, min_val, max_val) in enumerate(train_configs):
            ...
            if name == 'learning_rate':
                var = tk.DoubleVar(value=default)
                widget = tk.Spinbox(
                    train_frame, from_=min_val, to=max_val, 
                    increment=0.001, width=10, textvariable=var, format="%.4f"
                )
            else:
                var = tk.IntVar(value=default)
                widget = tk.Spinbox(
...
class TrainingTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.stop_button = ttk.Button(
            control_frame, text="Stop Training", command=self.stop_training, state='disabled'
        )
    ...
    def train_model(self, config: Config):
        ...
        try:
            ...
            train_loader, val_loader, data_info = create_data_loaders(
                text, vocab, config.training.seq_length, config.training.batch_size,
                config.training.validation_split
            )
            
            self.log_message(f"Data prepared: {data_info['total_chars']} characters, "
            ...
            self.log_message(f"Model created: {model.get_num_parameters()[0]:,} parameters")
            ...
            self.log_message(f"Final loss: {results.get('final_train_loss', 'N/A'):.6f}")
...
class GenerationTab(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        for i, (name, label, default, min_val, max_val) in enumerate(param_configs):
            ...
            if name in ['temperature', 'top_p']:
                ...
                widget = tk.Spinbox(
                    params_frame, from_=min_val, to=max_val, 
                    increment=0.1, width=10, textvariable=var, format="%.1f"
                )
            else:
                var = tk.IntVar(value=default)
                widget = tk.Spinbox(
        ...
        ttk.Button(
            control_frame, text="Clear", command=self.clear_output
        ).pack(side='left', padx=5)
        
        ttk.Button(
            control_frame, text="Save Output", command=self.save_output
        ).pack(side='left', padx=5)
    ...
    def generate_text(self):
        ...
        try:
            ...
            self.generator.config.sampling_method = SamplingMethod(self.sampling_method.get())
            
            # Recreate sampling strategy with new config
            self.generator.sampling_strategy = self.generator._create_sampling_strategy()
            ...
            def generate_in_thread():
                try:
                    ...
                    self.app.root.after(0, lambda: self.generation_completed(result))                except Exception as e:
                    self.app.root.after(0, lambda: self.generation_failed(str(e)))
